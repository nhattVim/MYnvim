...
local my_opts = {
    ...
    nvim = {
        ...
        foldexpr       = "v:lua.vim.treesitter.foldexpr()", --> native treesitter folding
...
if vim.fn.has("wsl") == 1 then
    ...
    if vim.fn.executable(win32yank) == 0 then
        ...
        local custom_path = local_bin .. "/win32yank.exe"

        if vim.fn.filereadable(custom_path) == 0 then
            vim.notify("Downloading win32yank.exe for WSL clipboard...", vim.log.levels.INFO, { title = "WSL Setup" })
            local cmd = string.format(
                "mkdir -p %s && curl -sLo /tmp/win32yank.zip https://github.com/equalsraf/win32yank/releases/download/v0.1.1/win32yank-x64.zip && unzip -p /tmp/win32yank.zip win32yank.exe > %s && chmod +x %s && rm /tmp/win32yank.zip",
                local_bin,
                custom_path,
                custom_path
            )
            ...
            if vim.v.shell_error ~= 0 then
                vim.notify("Failed to install win32yank.exe", vim.log.levels.ERROR, { title = "WSL Setup" })
            else
                vim.notify("win32yank.exe installed successfully!", vim.log.levels.INFO, { title = "WSL Setup" })
            end
        end
        win32yank = custom_path
    end
